...
public class SimpleDrive2 implements IState{

    private double XPosition, ZPosition = 0;
    private double posX = 0;
    private double speedX, speedZ = 0;
    private boolean turnOneSide = false;
    ...
    private double[][] speedZArrayJustTurn = { { 0, 1, 5, 10, 26, 60, 75, 90 }, 
                                               { 0, 15, 20, 25, 30, 45, 50, 70 } };

    private double[][] startKoefSpeedForX = { { 0.33, 0.66, 1 }, { 0.33, 0.66, 1 } };

    public SimpleDrive2(double XPosition, double ZPosition){
        this.XPosition = XPosition;
        this.ZPosition = ZPosition;
    }

    public SimpleDrive2(double ZPosition, boolean turnOneSide){
        this.ZPosition = ZPosition;
        this.turnOneSide = turnOneSide; 
    }
    ...
    public void execute() {
        ...
        if (XPosition != 0) {
            ...
            SmartDashboard.putNumber("DriveSimp", 1);
    
        } else {
            if (turnOneSide) {
                Main.motorControllerMap.put("useOneSide", 1.0);
                ...
                if (speedZ > 0) {
                    Main.motorControllerMap.put("speedX", speedZ * startKoef);
                } else {
                    Main.motorControllerMap.put("speedZ", speedZ * startKoef);
                }

                SmartDashboard.putNumber("DriveSimp", 2);
                ...
                SmartDashboard.putNumber("DriveSimp", 3);
            }
        }
    }
